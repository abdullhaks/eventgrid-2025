...
const Venues = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await getVenueService(currentPage, LIMIT);
  ...
  const handleView = async (id: string) => {
    try {
      const service = await getVenueServiceById(id);
  ...
  const handleAdd = async (formData: FormData) => {
    ...
    try {
      const response = await addVenueService(payload);
      await fetchData();
      return response;
    } catch (error) {
      throw error;
  ...
  const handleUpdate = async (formData: FormData) => {
    ...
    try {
      const response = await updateVenueService(selectedService._id, payload);
      await fetchData();
      return response;
    } catch (error) {
      throw error;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Venue Services</h2>
          <p className="text-sm text-gray-500 mt-1">Manage all venue service </p>
        </div>
