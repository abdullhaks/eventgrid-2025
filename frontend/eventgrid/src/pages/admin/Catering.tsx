...
const Catering = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await getCateringService(currentPage, LIMIT);
  ...
  const handleView = async (id: string) => {
    try {
      const service = await getCateringServiceById(id);
  ...
  const handleAdd = async (formData: FormData) => {
    ...
    try {
      const response = await addCateringService(payload);
      await fetchData();
      return response;
    } catch (error) {
      throw error;
  ...
  const handleUpdate = async (formData: FormData) => {
    ...
    try {
      const response = await updateCateringService(selectedService._id, payload);
      await fetchData();
      return response;
    } catch (error) {
      throw error;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Catering Services</h2>
          <p className="text-sm text-gray-500 mt-1">Manage all catering service </p>
        </div>
          ...
          <table className="w-full hidden md:table">
            <thead className="bg-gray-50 text-xs font-medium text-gray-500 uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-3 text-left">Chief Chef</th>
